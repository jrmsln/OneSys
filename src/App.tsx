    ...
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        <div className="content-wrap">
          {activeView === 'Services' && leaderAccess && session ? <ServicesView session={session} onBack={() => setActiveView('Dashboard')} /> : activeView === 'Members' ? <MembersView onBack={() => setActiveView('Dashboard')} /> : activeView === 'Calendar' && session ? <CalendarView session={session} onBack={() => setActiveView('Dashboard')} /> : activeView === 'Availability' && session ? <AvailabilityView session={session} onBack={() => setActiveView('Dashboard')} /> : activeView === 'Songs' && session ? <SongsView session={session} onBack={() => setActiveView('Dashboard')} /> : activeView === 'Archives' && leaderAccess ? <ArchiveView onBack={() => setActiveView('Dashboard')} /> : activeView === 'Profile' && session ? <ProfileView session={session} onBack={() => setActiveView('Dashboard')} installPrompt={installPrompt} installed={appInstalled} onInstall={() => void installApp()} /> : activeView === 'Admin' && leaderAccess ? <AdminView onBack={() => setActiveView('Dashboard')} /> : activeView !== 'Dashboard' ? <section className="empty-view"><p className="eyebrow">ONESYS WORKSPACE</p><h1>{activeView}</h1><p>This view is ready for the next build slice.</p><button className="primary-button" onClick={() => setActiveView('Dashboard')}>Back to dashboard</button></section> : <>
            <section className="welcome-row"><div><p className="eyebrow">{new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: '2-digit', year: 'numeric' }).toUpperCase()}</p><h1>{getTimeGreeting()}, {displayName.split(' ')[0]}.</h1><p className="subheading">Here is what is happening with your team this week.</p></div><div className="week-pill"><span className="pulse"></span>Week {Math.ceil(new Date().getDate() / 7)} <b>{new Date().toLocaleDateString('en-US', { day: '2-digit', month: 'short' })}</b></div></section>
